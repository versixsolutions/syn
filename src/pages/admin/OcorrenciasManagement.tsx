...
export default function OcorrenciasManagement() {
  ...
  const [selectedOcorrencia, setSelectedOcorrencia] =
    useState<OcorrenciaAdmin | null>(null);
  ...
  }, [selectedCondominioId, loadOcorrencias]);
  ...
  async function handleSaveUpdate(e: React.FormEvent) {
    ...
    try {
      ...
      setOcorrencias((prev) =>
        prev.map((o) =>
          o.id === selectedOcorrencia.id ? { ...o, ...updates } : o,
        ),
      );
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Acompanhe e resolva as solicitações dos moradores.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Nenhuma ocorrência encontrada com este filtro."
        />
      ) : (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start mb-1">
                  ...
                  <Tooltip
                    content={`Status: ${STATUS_OPTIONS.find((s) => s.value === oco.status)?.label || oco.status}`}
                  >
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase ${STATUS_OPTIONS.find((s) => s.value === oco.status)?.color || "bg-gray-100"}`}
                      aria-label={`Status da ocorrência: ${STATUS_OPTIONS.find((s) => s.value === oco.status)?.label || oco.status}`}
                    >
                      {STATUS_OPTIONS.find((s) => s.value === oco.status)
                        ?.label || oco.status}
                    </span>
                  </Tooltip>
                </div>
          ...
          <div className="space-y-6">
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
              ...
              <div className="flex gap-4 text-xs text-gray-500">
                <span>
                  <strong>Local:</strong> {selectedOcorrencia.location}
                </span>
                <span>
                  <strong>Categoria:</strong> {selectedOcorrencia.category}
                </span>
              </div>
              ...
              {selectedOcorrencia.photo_url && (
                <div className="mt-4">
                  ...
                  <a
                    href={selectedOcorrencia.photo_url}
                    target="_blank"
                    rel="noreferrer"
                  >
                    <img
                      src={selectedOcorrencia.photo_url}
                      alt="Evidência"
                      loading="lazy"
                      decoding="async"
                      className="w-full h-48 object-cover rounded-lg border border-gray-200 hover:opacity-90 transition"
                    />
                  </a>
                </div>
              )}
            ...
            <form onSubmit={handleSaveUpdate} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">
                  Notas Internas
                  <span className="ml-2 text-xs font-normal text-gray-400 bg-yellow-100 text-yellow-800 px-1.5 py-0.5 rounded">
                    Privado
                  </span>
                </label>
                <textarea
                  ...
                  onChange={(e) =>
                    setEditForm({ ...editForm, internal_notes: e.target.value })
                  }
                />
              </div>
