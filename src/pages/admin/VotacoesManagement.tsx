...
export default function VotacoesManagement() {
  ...
  }, [selectedCondominioId, loadVotacoes]);
  ...
  async function handleDelete(id: string) {
    if (!confirm("Tem certeza? Isso apagará todos os votos registrados!"))
      return;
    try {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Crie votações e acompanhe resultados.
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Crie uma nova pauta para decisão dos moradores."
        />
      ) : (
        <div className="grid gap-4">
          {votacoes.map((votacao) => (
            <div
              key={votacao.id}
              className={`bg-white p-5 rounded-xl shadow-sm border-l-4 transition relative group ${votacao.status === "ativa" ? "border-l-green-500" : "border-l-gray-300"}`}
            >
              ...
              <div className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                ...
                <div className="flex flex-wrap gap-2">
                  {Array.isArray(votacao.options) &&
                    votacao.options.map((opt: any) => (
                      <span
                        key={opt.id}
                        className="text-xs bg-white border border-gray-200 px-2 py-1 rounded text-gray-600 font-medium"
                      >
                        {opt.text}
                      </span>
                    ))}
